...
public class CrucibleBlock extends Block {
    ...
    public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
        ...
        if (!worldIn.isRemote && entityIn instanceof ItemEntity && i > 0 && entityIn.getPosY() <= (double)f) {
            ItemEntity item = (ItemEntity) entityIn;
            if (item.getItem().isItemEqual(new ItemStack(Items.REDSTONE)) && redstoneCount == 0) {
                redstoneCount += item.getItem().getCount();
                entityIn.remove();
            } else if (item.getItem().isItemEqual(new ItemStack(Items.DIAMOND)) && diamondCount == 0) {
                diamondCount += item.getItem().getCount();
                entityIn.remove();
            }
        }

        while (redstoneCount >= 1 && diamondCount >= 1) {
            ItemEntity output = new ItemEntity(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemInit.RED_CRYSTAL.get()));
            output.addVelocity(0, 2, 0);
            worldIn.addEntity(output);
            redstoneCount--;
            diamondCount--;
        }
    }
